      ...
      <div className="flex flex-col gap-6 md:flex-row items-center justify-between mb-10">
        <div className="space-y-1 text-center md:text-left">
          <div className="inline-flex items-center gap-2 bg-pink-100 text-pink-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mb-2">
            <FaCarSide size={14} /> Luxury Fleet
          </div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">
            Premium <span className="text-pink-600">Car Rental</span>
          </h2>
          <p className="text-slate-500 font-medium">Find the perfect ride for your grand entry</p>
        </div>

        {/* City Filter - Modern Toggle Style */}
      ...
      >
        {products && products.length > 0 ? (
                ...
                <div className="group relative bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl hover:shadow-pink-200 transition-all duration-500 overflow-hidden">
                  ...
                  <div className="relative h-72 overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-2xl shadow-lg">
                      <p className="text-pink-600 font-black text-xs">Featured</p>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <div className="flex items-center gap-1 bg-pink-50 px-2 py-1 rounded-lg">
                        ...
                        <span className="text-pink-700 font-bold text-xs">{item.ratings || "4.8"}</span>
                        <span className="text-pink-300 text-[10px] font-medium">(100+)</span>
                      </div>
                    ...
                    <button className="w-full mt-2 cursor-pointer py-3 bg-slate-900 text-white group-hover:bg-pink-600 rounded-xl font-bold text-xs uppercase tracking-widest transition-all duration-300 active:scale-95 shadow-lg group-hover:shadow-pink-200">
                      View Details
                    </button>
                  </div>
                </div>
          ...
          <div className="w-full py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center">
             ...
             <p className="text-xl font-bold text-slate-400">No cars available in {city}</p>
          </div>
        )}
      </Swiper>
