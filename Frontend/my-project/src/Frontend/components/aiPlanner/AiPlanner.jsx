  ...
  const getPriceDetails = (vendor, guestCount) => {
    ...
    if (type === "hall") {
      ...
      total =
        ...
        vendor.decoratorminPrice ||
        vendor.carRentalPrice ||
        0;
      isPerHead = false;
    }
    ...
    <div className="min-h-screen bg-[#030712] text-slate-100 font-sans selection:bg-pink-500/30">
      ...
      <div className="relative pt-24 pb-12 px-6 text-center z-10">
        ...
        <p className="text-slate-400 max-w-lg mx-auto text-sm md:text-base">
          Enter your requirements and let AI build the perfect vendor bundle for
          you.
        </p>
      </div>
      <p className="text-slate-500 text-sm mb-2 text-center">
        Example: "I want DJ, catering, hall booking, decorators booking ,
        photograhper booking and car rental. My total budget is 500,000 PKR and
        I have 120 guests (for hall/catering only)."
      </p>
      {/* Search Input */}
      <div className="max-w-2xl w-full mx-auto px-6 mb-20 relative z-10">
        <div className="bg-slate-900/80 backdrop-blur-xl border border-white/10 p-2 rounded-3xl shadow-2xl transition-all focus-within:border-pink-500/50 focus-within:ring-1 focus-within:ring-pink-500/20">
          {/* Textarea */}
          <textarea
            className="w-full bg-transparent px-6 py-4 outline-none text-white placeholder:text-slate-500 min-h-[120px] resize-none text-lg transition-all"
          ...
          <div className="flex items-center justify-between p-2 border-t border-white/5">
            ...
            <button
              ...
              className="bg-pink-600 cursor-pointer hover:bg-pink-500 disabled:opacity-50 disabled:cursor-not-allowed px-8 py-3 rounded-2xl font-bold transition-all flex items-center gap-2 text-white shadow-lg shadow-pink-600/20 active:scale-95"
            >
              {loading ? (
                <div className="animate-pulse">Planning...</div>
              ) : (
                <>
                  <FaMagic className="text-pink-200" /> Plan My Event
                </>
              )}
            </button>
          </div>
          ...
          <div key={i} className="mb-20">
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
              <h2 className="text-2xl font-bold flex items-center gap-3">
                <span className="bg-pink-600 text-white w-8 h-8 rounded-lg flex items-center justify-center text-sm">
                  #{i + 1}
                </span>
                Recommended Bundle
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {deal.services?.map((s, idx) => {
                const { perHead, total, isPerHead } = getPriceDetails(
                  s.vendor,
                  guests,
                );
                return (
                  <div
                    key={idx}
                    className="bg-slate-900/50 border border-white/5 rounded-3xl overflow-hidden hover:translate-y-[-5px] transition-all duration-300"
                  >
                    <div className="h-40 relative">
                      {s.vendor?.images?.[0] ? (
                        ...
                        <div className="w-full h-full bg-slate-800 flex items-center justify-center">
                          <FaRegImage className="opacity-20 text-3xl" />
                        </div>
                      )}
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex justify-between items-end">
                              ...
                              <span className="text-xl font-black text-white">
                                <span className="text-xs text-pink-500 mr-1">
                                  Rs.
                                </span>
